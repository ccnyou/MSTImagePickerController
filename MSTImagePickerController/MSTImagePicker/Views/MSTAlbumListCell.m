...
+ (instancetype)cellWithTableView:(UITableView *)tableView {
    ...
    if (!cell) {
        cell = [[MSTAlbumListCell alloc] initWithStyle:UITableViewCellStyleValue1
    ...
    if (config.isShowAlbumThumbnail) {
        ...
        if (_albumModel.count) {
            ...
            for (int i = 0; i < MIN(_albumModel.count, 3); i++) {
                [[PHCachingImageManager defaultManager] requestImageForAsset:_albumModel.content[fetchImageIndex]
                                                                  targetSize:CGSizeMake(80 * 2.f, 80 * 2.f)
                                                               ...
                                                               resultHandler:^(UIImage *_Nullable result, NSDictionary *_Nullable info) {
                                                                   ...
                                                                   if (i == MIN(self->_albumModel.count, 3) - 1) {
                                                                       dispatch_async(dispatch_get_main_queue(), ^{
                                                                           for (int j = 0; j < MIN(self->_albumModel.count, 3); j++) {
                                                                               switch (j) {
                                                                                   case 0:
...
- (NSArray *)mp_addLayoutContraints:(UIImageView *)imageView {
    ...
    NSLayoutConstraint *leading = [NSLayoutConstraint constraintWithItem:imageView
