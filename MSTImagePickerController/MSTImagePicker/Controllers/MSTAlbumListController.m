...
@interface MSTAlbumListController ()<PHPhotoLibraryChangeObserver> {
    PHFetchResult *_collectionResult;
}
...
- (void)mp_initData {
    ...
    //读取相册信息
    [[MSTPhotoManager defaultManager] loadAlbumInfoIsShowEmpty:config.isShowEmptyAlbum
                                                        isDesc:config.isPhotosDesc
                                               isOnlyShowImage:config.isOnlyShowImages
                                               CompletionBlock:^(PHFetchResult *customAlbum, NSArray *albumModelArray) {
                                                   self->_collectionResult = customAlbum;
                                                   ...
                                                   callBack:^(MSTImagePickerSourceType sourceType, MSTAuthorizationStatus status) {
                                                       dispatch_async(dispatch_get_main_queue(), ^{
                                                           if (status != MSTAuthorizationStatusAuthorized) {
                                                               ...
                                                               label.text = [NSString stringWithFormat:NSLocalizedStringFromTable(@"str_access_photo", @"MSTImagePicker", @"访问相册"),
                                                                                                       name];
                                                               ...
                                                               [btn setTitle:NSLocalizedStringFromTable(@"str_setting", @"MSTImagePicker", @"设置")
                                                                    forState:UIControlStateNormal];
                                                               btn.titleLabel.font = [UIFont systemFontOfSize:18];
                                                               btn.translatesAutoresizingMaskIntoConstraints = NO;
                                                               [btn addTarget:self
                                                                       action:@selector(mp_jumpToSettings)
                                                             forControlEvents:UIControlEventTouchUpInside];

                                                               [self.view addSubview:btn];

                                                               NSLayoutConstraint *center1 = [NSLayoutConstraint constraintWithItem:btn
                                                                                                                          attribute:NSLayoutAttributeCenterX
                                                                                                                          relatedBy:NSLayoutRelationEqual
                                                                                                                             toItem:self.view
                                                                                                                          attribute:NSLayoutAttributeCenterX
                                                                                                                         multiplier:1
                                                                                                                           constant:0];
                                                               NSLayoutConstraint *top1 = [NSLayoutConstraint constraintWithItem:btn
                                                                                                                       attribute:NSLayoutAttributeTop
                                                                                                                       relatedBy:NSLayoutRelationEqual
                                                                                                                          toItem:label
    ...
    if (config.isShowAlbumThumbnail) {
        ...
        if (!cell) {
            cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleValue1
                                          reuseIdentifier:kAlbumCellReuserIdentifier];
        }
...
- (void)photoLibraryDidChange:(PHChange *)changeInstance {
    // 检测是否有资源变化
    PHFetchResultChangeDetails *collectionChanges = [changeInstance changeDetailsForFetchResult:_collectionResult];
