...
- (void)mp_setupViews {
    ...
    [self.collectionView registerClass:[MSTPhotoGridHeaderView class]
            forSupplementaryViewOfKind:UICollectionElementKindSectionHeader
                   withReuseIdentifier:@"MSTPhotoGridHeaderView"];
    if (_isShowCamera)
        [self.collectionView registerClass:[MSTPhotoGridCameraCell class]
                forCellWithReuseIdentifier:@"MSTPhotoGridCameraCell"];
...
- (void)mp_refreshMoments {
    _momentsArray = nil;
    _momentsArray = [[MSTPhotoManager defaultManager] sortByMomentType:self.config.photoMomentGroupType
                                                                assets:_album.models];
}
...
- (void)mp_scrollToBottom {
    ...
    [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:item inSection:section]
                                atScrollPosition:UICollectionViewScrollPositionBottom
                                        animated:NO];
}

- (MSTPhotoGridCameraCell *)mp_addCameraCell:(UICollectionView *)collectionView indexPath:(NSIndexPath *)indexPath {
    MSTPhotoGridCameraCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"MSTPhotoGridCameraCell"
...
- (void)mp_didFinishMakingVideoWithAsset:(PHAsset *)asset url:(NSURL *)url {
    ...
    if ([pickerCtrler.MSTDelegate respondsToSelector:@selector(MSTImagePickerController:didFinishPickingVideoWithURL:identifier:)]) {
        [pickerCtrler.MSTDelegate MSTImagePickerController:pickerCtrler
                              didFinishPickingVideoWithURL:url
...
// #warning !!!Waiting for updating 代码优化
    ...
    MSTPhotoGridCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:reuserIdentifier
                                                                       forIndexPath:indexPath];
    cell.delegate = self;
    cell.asset = model;
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView
    ...
    MSTMoment *moment = _momentsArray[indexPath.section];
    MSTPhotoGridHeaderView *header = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader
                                                                        withReuseIdentifier:@"MSTPhotoGridHeaderView"
                                                                               forIndexPath:indexPath];
    ...
    if (pushToCamera) {
        ...
        if (model.type == MSTAssetModelMediaTypeVideo) {
            ...
            if ([pickerCtrler hasSelected]) {
                [self presentViewController:[self addAlertControllerWithTitle:NSLocalizedStringFromTable(@"str_both_video_photo", @"MSTImagePicker", @"不能同时选择视频和照片")
...
// #warning waiting for updating 虽然有一种方法搞定了。但是还是感觉会有更好的办法。
            if ([collectionChanges hasIncrementalChanges]) {
                BOOL isCamera = self->_isShowCamera && self.config.isPhotosDesc;
                ...
                NSInteger item = self->_isShowCamera ? removedPaths.lastObject.item - 1 : removedPaths.lastObject.item;
    ...
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        ...
        if ([mediaType isEqualToString:(NSString *)kUTTypeImage]) {
            ...
            if (self.config.customAlbumName.length) {
                [[MSTPhotoManager defaultManager] saveImageToCustomAlbumWithImage:image
                                                                        albumName:self.config.customAlbumName
                                                                  completionBlock:^(PHAsset *asset, NSString *error) {
                                                                      if (error.length) NSLog(@"Save image to custom album error: %@", error);
                                                                  }];
            } else {
                //保存到系统相册
                [[MSTPhotoManager defaultManager] saveImageToSystemAlbumWithImage:image
                                                                  completionBlock:^(PHAsset *asset, NSString *error) {
            ...
            if (self.config.isVideoAutoSave) {
                if (self.config.customAlbumName.length) {
                    //保存到自定义相册
                    [[MSTPhotoManager defaultManager] saveVideoToCustomAlbumWithURL:url
                                                                          albumName:self.config.customAlbumName
                                                                    completionBlcok:^(PHAsset *asset, NSString *error) {
                                                                        if (error) {
                                                                            NSLog(@"Save video to custom album error: %@", error);
                                                                        } else {
                                                                            [self mp_didFinishMakingVideoWithAsset:asset
                                                                                                               url:url];
                                                                        }
                                                                    }];
                } else {
                    [[MSTPhotoManager defaultManager] saveVideoToSystemAlbumWithURL:url
                                                                    completionBlock:^(PHAsset *asset, NSString *error) {
                                                                        if (error) {
