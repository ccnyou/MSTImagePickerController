...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(mp_appWillRisgnActive)
...
- (void)mp_setupSubviews {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(mp_endPlaying)
...
- (void)mp_playButtonDidClicked:(UIButton *)sender {
    if (!_isPlay) {
        ...
        [_playButton setImage:[UIImage imageNamed:@"icon_preview_video_play_highlighted"]
...
- (void)mp_doneButtonDidClicked:(UIButton *)sender {
    ...
    if ([pickerCtrler.MSTDelegate respondsToSelector:@selector(MSTImagePickerController:didFinishPickingVideoWithURL:identifier:)]) {
        [[PHImageManager defaultManager] requestAVAssetForVideo:_asset
                                                        options:nil
                                                  resultHandler:^(AVAsset *_Nullable asset, AVAudioMix *_Nullable audioMix, NSDictionary *_Nullable info) {
                                                      AVURLAsset *urlAsset = (AVURLAsset *)asset;
                                                      [pickerCtrler.MSTDelegate MSTImagePickerController:pickerCtrler
                                                                            didFinishPickingVideoWithURL:urlAsset.URL
                                                                                              identifier:self->_asset.localIdentifier];
                                                  }];
    }
}
...
- (UIButton *)playButton {
    if (!_playButton) {
        ...
        [_playButton addTarget:self
                        action:@selector(mp_playButtonDidClicked:)
              forControlEvents:UIControlEventTouchUpInside];

        [self.view addSubview:_playButton];

        NSLayoutConstraint *leading = [NSLayoutConstraint constraintWithItem:_playButton
                                                                  ...
                                                                  multiplier:1
                                                                    constant:0];
        NSLayoutConstraint *top = [NSLayoutConstraint constraintWithItem:_playButton
                                                              ...
                                                              multiplier:1
                                                                constant:0];
        NSLayoutConstraint *trailing = [NSLayoutConstraint constraintWithItem:_playButton
                                                                   ...
                                                                   multiplier:1
                                                                     constant:0];
        NSLayoutConstraint *bottom = [NSLayoutConstraint constraintWithItem:_playButton
