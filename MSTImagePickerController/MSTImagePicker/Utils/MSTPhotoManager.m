...
+ (void)checkAuthorizationStatusWithSourceType:(MSTImagePickerSourceType)type
                                      callBack:(void (^)(MSTImagePickerSourceType, MSTAuthorizationStatus))callBackBlock {
    switch (type) {
...
- (void)loadCameraRollInfoIsDesc:(BOOL)isDesc
    ...
    PHFetchResult *albumCollection = [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum
...
- (void)loadAlbumInfoIsShowEmpty:(BOOL)isShowEmpty
                          isDesc:(BOOL)isDesc
                 isOnlyShowImage:(BOOL)isOnlyShowImage
                 CompletionBlock:(void (^)(PHFetchResult *, NSArray *))completionBlock {
    //用来存放每个相册的model
    ...
    [self loadCameraRollInfoIsDesc:isDesc
                       isShowEmpty:isShowEmpty
                   isOnlyShowImage:isOnlyShowImage
...
- (void)saveImageToSystemAlbumWithImage:(UIImage *)image
                        completionBlock:(void (^)(PHAsset *, NSString *))completionBlock {
...
- (void)saveVideoToCustomAlbumWithURL:(NSURL *)url
                            albumName:(NSString *)albumName
                      completionBlcok:(void (^)(PHAsset *, NSString *))completionBlock {
    __weak typeof(self) weakSelf = self;
...
- (NSArray<MSTMoment *> *)sortByMomentType:(MSTImageMomentGroupType)momentType assets:(NSArray *)models {
    ...
    for (NSInteger i = 0; i < models.count; i++) {
        ...
        NSDateComponents *components = [[NSCalendar currentCalendar] components:NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitWeekday
                                                                       fromDate:asset.asset.creationDate];
...
- (void)getMSTAssetModelWithPHFetchResult:(PHFetchResult *)fetchResult
                          completionBlock:(void (^)(NSArray<MSTAssetModel *> *))completionBlock {
...
- (void)getThumbnailImageFromPHAsset:(PHAsset *)asset
                     ...
                     completionBlock:(void (^)(UIImage *, NSDictionary *))completionBlock {
    PHImageRequestOptions *options = [[PHImageRequestOptions alloc] init];
    options.resizeMode = PHImageRequestOptionsResizeModeFast;
    options.synchronous = NO;

    [self mp_getImageFromPHAsset:asset
                       imageSize:CGSizeMake(width * 2.f, width * 2.f)
                         options:options
                isFixOrientation:NO
...
- (void)getPreviewImageFromPHAsset:(PHAsset *)asset
                   ...
                   completionBlock:(void (^)(UIImage *, NSDictionary *, BOOL))completionBlock {
...
- (void)mp_getImageFromPHAsset:(PHAsset *)asset
                     imageSize:(CGSize)imageSize
                       options:(PHImageRequestOptions *)options
              isFixOrientation:(BOOL)fixOrientation
...
- (void)getImageBytesWithArray:(NSArray<MSTAssetModel *> *)models
    ...
    for (MSTAssetModel *model in models) {
        [self.imageManager requestImageDataForAsset:model.asset
                                            options:options
                                      resultHandler:^(NSData *_Nullable imageData, NSString *_Nullable dataUTI, UIImageOrientation orientation, NSDictionary *_Nullable info) {
                                          count--;
                                          dataLength += imageData.length;
                                          if (count <= 0) {
                                              completionBlock ? completionBlock([NSByteCountFormatter stringFromByteCount:dataLength
                                                                                                               countStyle:NSByteCountFormatterCountStyleFile]) : nil;
                                          }
                                      }];
    }
}
